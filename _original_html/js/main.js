...
function documentReadyInit() {
	...
	$datepicker.on('click', function() {
		var datepickerWidth = $datepicker.parent().width();
		jQuery('#ui-datepicker-div').width(datepickerWidth);
	});
	...
	// var owl1 = jQuery('.testimonials-owl-dots');
	// var owl2 = jQuery('.testimonials-owl-content');
...
function windowLoadInit() {
	...
	if ($header.length) {
		...
		if ($header.hasClass('header_transparent')) {
			jQuery('.page_header_wrapper').addClass('header_transparent_wrapper');
		}
...
/* overlapped owl nav width calculation */
function overlapOwlNavWidth() {
	jQuery('.overlapped-owl-nav').each(function(){
		var $carousel = jQuery(this);
